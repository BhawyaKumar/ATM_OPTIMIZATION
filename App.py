...
@st.cache_data
def load_data():
    df_ATM = pd.read_csv("ATM_Cash_management.csv")
    ...
    if 'Holiday_Flag' in df_ATM.columns:
        df_ATM['Holiday_Flag'] = pd.to_numeric(df_ATM['Holiday_Flag'], errors='coerce').fillna(0).astype(int)
    if 'Special_Event_Flag' in df_ATM.columns:
        df_ATM['Special_Event_Flag'] = pd.to_numeric(df_ATM['Special_Event_Flag'], errors='coerce').fillna(0).astype(int)

    for c in ['ATM_ID','Day_of_Week','Time_of_Day','Weather_Condition']:
        if c in df_ATM.columns:
            df_ATM[c] = df_ATM[c].astype(str).str.strip()
    return df_ATM
